...
import java.sql.{Connection, Date, Driver, DriverManager, PreparedStatement, ResultSet, SQLException, Timestamp}
...
case class ClickhousePartition(url: String, idx: Int ) extends Partition  {
  override def index: Int = idx
}

object ClickhouseRDD extends Logging {
...
/**
  ...
  */
private[jdbc] class ClickhouseRDD(
                                   sc: SparkContext,
                                   schema: StructType,
  ...
  /**
    ...
    */
  override def getPartitions: Array[Partition] = partitions   // jungle add : will be called in submitMissingTasks
  ...
  private val filterWhereClause: String =
    filters
      .flatMap(ClickhouseRDD.compileFilter(_, JdbcDialects.get(url)))
  ...
  */
  ...
  /*
  private def  getConnection(part : ClickhousePartition) : Connection = {

    Class.forName("ru.yandex.clickhouse.ClickHouseDriver")
    DriverManager.getConnection(part.url, "", "")
  ...
  */
  ...
  override def compute(thePart: Partition, context: TaskContext): Iterator[InternalRow] = {
    ...
    val part = thePart.asInstanceOf[ClickhousePartition]
    conn = getConnection(part).apply()
    val dialect = JdbcDialects.get(url)
    ...
    val sqlText = s"SELECT $columnList FROM ${options.table} "


    logInfo(" #### JDBCRDD sqlText is : " + sqlText  + ", connect url is :" +  part.url  )
